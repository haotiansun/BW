...
public class Script {
    ...
    public Script(String scr) {
        ...
        if (isScriptLegal(clauseList)) {
            addOnClickAction();
            addOnEnterAction();
            addOnDropAction();
        } else {
            System.out.println("Script is not legal. There should be at lease and only ONE trigger for each clause.");
        }
    ...
    public Script(Script scr) {
        this.scriptStr = scr.scriptStr;
        String[] splitedScript = this.scriptStr.split(";");
        for (String str: splitedScript) {
            str.trim();
            this.clauseList.add(str);
        }

        if (isScriptLegal(this.clauseList)) {
            addOnClickAction();
            addOnEnterAction();
            addOnDropAction();
        } else {
